...
function Template(props) {
  ...
  const adicionarDados = () => {
    if (item && quantidade > 0) {
      setLista([...lista, { produto: item, quantidade: Number(quantidade) }]);
      setItem('');
      setQuantidade('');
      setAbrirPopup(false);
    } else {
      alert('Por favor, insira um produto válido e uma quantidade positiva.');
    }
  };
  ...
  useEffect(() => {
    const total = lista.reduce((acc, curr) => acc + curr.quantidade, 0);
    setSoma(total);
  }, [lista]);
